    ...
    def update_dish_availability(self, dish_id, availability):
        ...
        for dish in self.menu_data:
            if dish['id'] == dish_id:
                ...
                self.save_menu_to_file()
                return {'msg':"availability updated!",'updated_dish':{**dish}}
        if not dish_found:
            return {"msg":"dish not found!"}
...
def get_menu():
    menu_data = menu_manager.get_menu()
...
def add_dish():
    dish = request.json
     # Validate dish data
    if 'id' not in dish or 'dish_name' not in dish or 'price' not in dish or 'availability' not in dish or 'stock' not in dish:
        return jsonify({"msg": "Please prived all fields!"}), 400

    menu_manager.add_dish(dish)
...
def update_availability(id):
    ...
    response =  menu_manager.update_dish_availability(id,availability)
